let handler = async (m, { conn }) => {
	...
	try {
		...
		if (global.db.data.datas.rowner.map(([number]) => number).map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(who) || who == conn.user.jid) return m.reply(`[ ! ] Tidak dapat ban *real owner*.`)
